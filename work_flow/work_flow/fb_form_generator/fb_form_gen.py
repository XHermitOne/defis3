...
def gen_wxfb_prj(parent=None, resource=None, fbp_filename=None):
    ...
    if fbp_filename is None:
        fbp_filename = dlgfunc.getFileDlg(parent,
    ...
    if os.path.exists(fbp_filename):
        if not dlgfunc.openAskBox(u'ВНИМАНИЕ', u'Файл <%s> уже существует. Перезаписать его?' % fbp_filename):
    ...
    if not generator.isParsed(resource):
        dlgfunc.openWarningBox(u'ОШИБКА', u'Тип ресурса не поддерживается генератором форм wxFormBuilder')
        return False
