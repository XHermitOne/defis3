...
# -*- coding: utf-8 -*-
"""
Resource module <C:/defis/NSI/NSI/spravEditDlgXP.frm>.
"""
### RESOURCE_MODULE: C:/defis/NSI/NSI/spravEditDlgXP.frm
# -----------------------------------------------------------------------------
# Name:        C:/defis/NSI/NSI/spravEditDlgXP_frm.py
...
def onCodControl(obj):
    ...
    try:
        ...
        prev_change_code= grid.getNameValue('cod')#dataset.getNameValue('cod')
        ...
        if not ctrl_ret in [coderror.IC_CTRL_OK,coderror.IC_CTRL_REPL]:
            dlgfunc.openMsgBox(u'ВНИМАНИЕ!', u'Код %s есть уже в справочнике!' % value)
...
def _get_refspr_parent_cod(sprav, cod):
    ...
    #print ' ---- ref_level:', lev.getIndex(), lev.getRefLevel(), type(lev.getRefLevel())
    rl = lev.getRefLevel()+1
    ix = lev.getIndex()+1
    beg = ix - rl
    old = cod_lst
    cod_lst = [el for el in cod_lst[beg:ix] if el]# + [None]
    #print '***** cod_lst:', old, cod_lst, ix, beg, rl
    return cod_lst
...
def onCodHlpSprav(evalSpace):
    ...
    if ref_sprav:
        ...
        #print '*** RESULT:', (res, cod, cod+new_cod, flds)
...
#        sprav.getStorage().setLevelTable(cur_cod,tab)
        buff=grid.GetTable().GetDataset().allChangeBuff
        #print '   ... buff:', buff.update_rows_dct
        _tab = sprav.getStorage()._tab
        #print ' $$$$$55 ', [x['activate'] for x in _tab.getResource()['child']]
...
def _onInitSpravTree(obj, bRefresh=True):
    ...
    try:
        ...
        sprav=evalSpace[CONTEXT_SPRAV_NAME]
        _tab = sprav.getStorage()._tab
        print(' $$$$$  _onInitSpravTree:', [x['activate'] for x in _tab.getResource()['child']])
