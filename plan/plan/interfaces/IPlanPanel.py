...
class IPlanPanel(icobjectinterface.icObjectInterface):
    ...
    def selectedFuncplansTreeCtrl(self, evt):
        ...
        if 1:   # data._edit_form:
            ...
            if data.isLock():
                msgbox.MsgBox(split,
                              u'Элемент плана заблокирован пользлвателем <%s>' % data.ownerLock())
            else:
                # print '????? >>>> lock() ??????'
                data.lock()
...
#                # print '<<<<< After >>>>>'
                if self.ILeftPanel.metatype not in IPlanPanel.edtPanelBuff:
            ...
            if data.value.metatype in IPlanPanel.edtPanelBuff:
