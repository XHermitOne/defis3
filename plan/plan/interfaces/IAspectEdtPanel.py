...
class IAspectEdtPanel(ieditpanel.IEdtPanel):
    ...
    def _findFullCode(self, metaObj, mType):
        ...
        while 1:
            ...
            if isinstance(metaObj.value.sprav, str):
...
#            self.LoadChildData()
    ...
    def hlpFunccodPlan(self, evt, grid, row, col):
        ...
        """
        # log.debug('===== len(grid.dataset.data[row]) = %s', len(grid.GetDataset().data[row]), row, COL_GRID_KEY)
