...
ic_class_doc = 'ic/doc/_build/html/ic.components.user.ic_user_wrp.html'
    ...
    if attr in ('main_win',):
        ...
        if ret:
            ...
            if not ret[0][0] in ('MainWindow', 'AUIMainWindow'):
                dlgfunc.openWarningBox(u'ОШИБКА', u'Выбранный объект не является главным окном.')
        ...
        if ret:
            ...
            for cur_psp in ret:
                if not cur_psp[0][0] in ('MenuBar', 'FlatMenuBar'):
                    dlgfunc.openWarningBox(u'ОШИБКА', u'Выбранный объект [%s] не является главным меню.' % cur_psp)
                    return coderror.IC_CTRL_FAILED_IGNORE
                # Все горизонтальные  меню должны быть одного типа
                if cur_psp[0][0] != first_menubar_type:
                    dlgfunc.openWarningBox(u'ОШИБКА', u'Ошибка типа. Обект [%s].' % cur_psp)
                    return coderror.IC_CTRL_FAILED_IGNORE
            return coderror.IC_CTRL_OK
    elif attr in ('password',):
...
class icUser(icwidget.icSimple, icuser.icUser):
    ...
    def __init__(self, parent, id=-1, component=None, logType=0, evalSpace=None,
        ...
        icuser.icUser.__init__(self, filefunc.getPathFile(glob_functions.getVar('SYS_RES'),
