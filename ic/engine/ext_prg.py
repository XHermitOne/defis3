...
EXTERNAL_PROGRAMS_CFG в формате:
    {
        'имя программмы': {
...
DEFIS_START = os.path.join(os.path.dirname(os.path.dirname(FILE_PKG_DIRNAME)), 'editor.sh')

EXTERNAL_PROGRAMS_CFG = {'calculator': {'description': u'Калькулятор',
                         ...
                         'pyreditor': {'description': u'PyReditor. Редактор регулярных выражений',
                                       ...
                                       'windows_cmd': None},
                         'defis': {'description': u'DEFIS',
                                   'linux_cmd': 'gnome-terminal -- \'%s\'' % DEFIS_START,
                                   'windows_cmd': None},
...
def run_external_programm(programm_name=None, run_cmd=None):
    ...
    if run_cmd:
        ...
        desc_to_name = dict(
            [(prg.get('description', u''), prg_name) for prg_name, prg in EXTERNAL_PROGRAMS_CFG.items()])
