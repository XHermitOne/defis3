...
class icMainWindow(wx.Frame):
    ...
    def __init__(self, name, win_struct, res_filename='',
        ...
        if RES_WIN_TITLEFUNC in win_struct and \
           not execfunc.is_empty_method(win_struct[RES_WIN_TITLEFUNC]):
            win_title = execfunc.execute_method(win_struct[RES_WIN_TITLEFUNC])
        else:
        ...
        try:
            ...
            if self._PaintFunc:
                ...
                if (self._MainNotebook is None) or not self._MainNotebook.IsShown():
                    execfunc.execute_method(self._PaintFunc, self)
    ...
    def OnOpen(self, event):
        ...
        try:
            ok = True
            if not execfunc.is_empty_method(self._OnOpen) and \
                not self._is_opened:
                ok = execfunc.execute_method(self._OnOpen, self)
    ...
    def OnClose(self, event):
        ...
        try:
            ok = True
            if not execfunc.is_empty_method(self._OnClose):
                ok = execfunc.execute_method(self._OnClose, self)
    ...
    def SetICTitle(self, title=''):
        ...
        if title != '':
            ...
            if self._TitleReadOnly == 0 and self._TitleFunc != {}:
                new_title = execfunc.execute_method(self._TitleFunc, self)
    ...
    def RefreshTitle(self):
        ...
        """
        try:
            if not execfunc.is_empty_method(self._TitleFunc):
                win_title = execfunc.execute_method(self._TitleFunc)
