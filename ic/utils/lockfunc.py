...
def DelMyLockInDir(lock_id, lock_dir, lock_filenames):
    """
    ...
    :param lock_filenames: Имена файлов и папок в директории lock_dir
    """
    try:
        ...
        log.fatal(u'Ошибка удаления файлов блокировки. Папка блокировки <%s>' % lock_dir)


def DelMyLock(lock_id=None, lock_dir=LOCK_DIR):
    """
    ...
    """
    if not lock_id:
        lock_id = GetMyHostName()
    return os.path.walk(lock_dir, DelMyLockInDir, lock_id)
...
def UnLockFile(filename, **unlock_if):
    """
    Разблокировать файл.

    :param filename: Имя файла.
    :param unlock_if: Условие проверки разблокировки.
    ...
    log.info(u'Сброс блокировки. Файл <%s> (%s). Условия снятия блокировки %s' % (lock_file,
                                                                                  os.path.exists(lock_file), unlock_if))
    if os.path.exists(lock_file):
        if unlock_if:
            lck_rec = ReadLockRecord(lock_file)
            # Если значения по указанным ключам равны, то все ОК
            can_unlock = bool(len([key for key in unlock_if.keys() if lck_rec.setdefault(key, None) == unlock_if[key]]) == len(unlock_if))
...
def UnLockAllFile(lock_dir, computer_name=None, user_name=None):
    """
    Разблокировка всех файлов.

    :param lock_dir: Директория блокировок.
    ...
    if not user_name:
        ...
        user_name = ic.engine.glob_functions.getVar('UserName')
    if lock_dir:
...
class icLockSystem:
    ...
    def __init__(self, lock_dir=None):
        ...
        :param lock_dir: Папка блокировки.
        """
        if lock_dir is None:
            lock_directory = getLockDir()
            lock_dir = lock_directory if os.path.exists(lock_directory) else LOCK_DIR

        log.info(u'Система блокировки. Используется директория блокировки <%s>' % lock_dir)
        self._LockDir = lock_dir
    ...
    def _getLockFileName(self, lock_name):
        ...
        try:
            if isinstance(lock_name, list):
                lockname = lock_name[-1]
            elif isinstance(lock_name, str):
                lockname = os.path.splitext(os.path.basename(lock_name))[0]
            lock_file_name = os.path.join(self._LockDir, lockname+LOCK_FILE_EXT)
    ...
    def lockFileRes(self, lock_name, lock_record=None, bUnLockMy=True):
        """
        ...
        :param lock_record: Запись блокировки.
        :param bUnLockMy: Автоматическая разблокировка моих блокировок?
        """
        lock_file_name = self._getLockFileName(lock_name)
        if lock_record is None:
            import ic.engine.glob_functions
            lock_record = {'computer': ComputerName(),
                        'user': ic.engine.glob_functions.getVar('UserName')}

        if bUnLockMy and self.isMyLockRes(lock_name, lock_record):
            # Это мной поставленная блокировка. Поэтому мы можем снять ее
            self.unLockFileRes(lock_name)
    ...
    def isMyLockRes(self, lock_name, lock_record=None):
        """
        ...
        lock_rec = self.getLockRec(lock_name)
        return lock_rec == lock_record

    def isLockFileRes(self, lock_name, bUnLockMy=True):
        """
        ...
        :param bUnLockMy: Автоматическая разблокировка моих блокировок?
        """
        if bUnLockMy and self.isMyLockRes(lock_name):
            # Это мной поставленная блокировка. Поэтому мы можем снять ее
            self.unLockFileRes(lock_name)
    ...
    def getLockUsername(self, lock_name):
        """
        Определить имя пользователя блокировки.

        :param lock_name: Имя блокировки.
        """
        if self.isLockFileRes(lock_name):
            lock_rec = self.getLockRec(lock_name)
            return lock_rec.get('user', UNKNOWN_USER)
        return UNKNOWN_USER

    def getLockComputer(self, lock_name):
        """
