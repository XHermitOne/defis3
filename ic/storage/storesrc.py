...
DEFAULT_STORAGE_DIR = filefunc.getProfilePath()
FILE_STORAGE_EXT = '.db'
SYSTEM_DIR_PREFIX = '#'
...
class icFileStorage(storage_interface.icElementStorageInterface):
    ...
    def renameFile(self, NewFileName_):
        ...
        """
        if NewFileName_ and os.path.exists(self._FileName):
            self.closeFile()
            os.rename(self._FileName, NewFileName_)
    ...
    def Clone(self, CloneName_):
        ...
        if not os.path.exists(new_file_name):
            self.closeFile()
            filefunc.copyFile(self._FileName, new_file_name, False)
...
class icDirStorage(storage_interface.icElementStorageInterface):
    ...
    def Open(self):
        ...
        list_dir = os.listdir(self._NodeDir)
    ...
    def makeNodeDir(self, NodeDir_=None):
        ...
        if not os.path.exists(self._NodeDir):
            filefunc.makeDirs(self._NodeDir)
    ...
    def Clone(self, CloneName_):
        ...
        if not os.path.exists(new_dir_name):
            self.closeAllFiles()
            filefunc.cloneDir(self._NodeDir, new_dir_name, False)
