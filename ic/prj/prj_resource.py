...
class icPrjDBRes(icPrjResource):
    ...
    def createResClass(self):
        ...
        # Сначал спросить какую БД будем создавать а затем создать ее
        global DATABASE_TYPE_CHOICE
        self.template = DATABASE_TYPE_CHOICE[dlgfunc.getSingleChoiceDlg(self.getRoot().getParent(),
    ...
    def extend(self):
        ...
        yes = dlgfunc.openAskBox(u'Проверка связи с БД', u'Поверить связь с БД <%s : %s>?' % (spc['name'], spc['type']))
        if yes:
            ...
            dlgfunc.openMsgBox(u'Проверка связи с БД', msg)
...
class icPrjPostgreSQLRes(icPrjResource):
    ...
    def extend(self):
        ...
        """
        # Данном случае проверка связи с БД
        yes = dlgfunc.openAskBox(u'Поверить связь с БД?')
        if yes:
            from ic.db import icdb
            res = self.getMyRes()
...
class icPrjWinRes(icPrjResource):
    ...
    def createResClass(self):
        ...
        # Сначал спросить какую БД будем создавать а затем создать ее
        global WINDOW_TYPE_CHOICE
        self.template = WINDOW_TYPE_CHOICE[dlgfunc.getSingleChoiceDlg(self.getRoot().getParent(),
...
class icPrjMenuRes(icPrjResource):
    ...
    def createResClass(self):
        ...
        # Сначал спросить какую меню будем создавать а затем создать ее
        global MENU_TYPE_CHOICE
        self.template = MENU_TYPE_CHOICE[dlgfunc.getSingleChoiceDlg(self.getRoot().getParent(),
                                                                    _('Choose menu type'),
                                                                    _('Menu type list:'),
                                                                    [txt for txt in MENU_TYPE_CHOICE.keys() if isinstance(txt, str)])]
        # Создать
