    ...
    def multiScan(self, scan_filename=None, n_page=-1):
        ...
        try:
            ...
            if n_page < 0:
                ...
                log.debug(u'Сканирование всех возможных страниц')
                is_stop_scan = False
                i_page = 0
                while not is_stop_scan:
                    image = None
                    try:
                        if hasattr(scan, 'next'):
                            image = scan.next()
                        else:
                            image = next(scan)
                    ...
                    result = self._imageDrawCanvas(image, scan_canvas, i_page) if image is not None else False
                    if not result:
                        continue
                ...
                log.debug(u'Сканирование определенное кол-во страниц')
                pages_scan_result = False
                for i_page in range(n_page):
                    ...
                    try:
                        ...
                        continue
                    result = self._imageDrawCanvas(image, scan_canvas, i_page) if image is not None else False
                    pages_scan_result = result
                    if not result:
                        continue
                if not pages_scan_result:
                    log.warning(u'Ошибка мульти-сканирования [%d] страниц' % n_page)
                    return False
            # Сохранить PDF холст
            log.debug(u'Контроль счетчика страниц [%d из %d]' % (i_page + 1, n_page))
            scan_canvas.save()
            return True
        except:
    ...
    def scan_pack(self, scan_filenames=()):
        ...
        for scan_filename, n_pages, is_duplex in scan_filenames:
            ...
            if tray_sheet_count <= max_sheets:
                log.debug(u'Обычное сканирования документа <%s>. Количество страниц [%d (%s)] Текущий счетчик %d. ' % (scan_filename, n_pages, is_duplex, tray_sheet_count))
            ...
            elif tray_sheet_count > max_sheets and (not result or (result[-1] is not None)):
