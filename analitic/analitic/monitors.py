...
class icMonitorNode:
    ...
    def view(self, bIndicator=False):
        ...
        """
        monitor_dict = NSI.spravfunc.FSprav(self.sprav_type, self.cod,
                                            ['name', 's1', 's2', 's3'])
        ...
        if monitor_dict['s2']:
            if issubclass(self.parent_win.__class__, wx.Notebook):
                ...
                self.parent_win.AddPage(monitor_form, monitor_dict['name'], select=True)
    ...
    def viewLink(self, LinkAlias_):
        ...
        if monitor_dict['s4'].strip():
            ...
            if LinkAlias_ in links:
                monitor_node = icMonitorNode(links[LinkAlias_], self.parent_win)
    ...
    def loadBody(self):
        ...
        """
        monitor_dict = NSI.spravfunc.FSprav(self.sprav_type, self.cod, ['name', 's3'])
        try:
    ...
    def saveBody(self, Body_=None):
        ...
        monitor_id = NSI.spravfunc.getSpravCodeId(self.sprav_type, self.cod)
        if monitor_id:
    ...
    def newMonitor(self, ViewForm_=None, WizardForm_=None, Body_=None):
        ...
        try:
            ...
            log.fatal(u'Ошибка создания монитора')
            return None
