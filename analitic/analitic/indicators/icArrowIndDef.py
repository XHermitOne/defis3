...
def _get_day_plan(date, cod):
    ...
    if ret is not None and cod is not None:
        ...
        return ret
    elif _buff_last_value.has_key(cod):
    ...
    for r in rs:
        ...
        if (t1 is None and t2 is None) or (t1 <= tm <=t2):
            try:
                ...
                print('### TYPE ERROR par=', par)
                print('### getattr(r, par)=', getattr(r, par))
                
            if parPlan:
                # print '>>>> !!!! parPlan:', parPlan, getattr(r, parPlan)
                v = getattr(r, parPlan)
                ...
                #   Если плановое значение не определено
                    ...
                    if funcPlan:
                        yy, mm, dd = map(lambda x: int(x), tm.split('.'))
...
def _disp_func_agr(rs, parTime, par, parPlan=None, t1=None, t2=None,
    ...
    if count>0:
        ...
        if parPlan:
            print('>>> DISPERSION=', dsp/count, dspPlan/count)
