...
def getSpravBuffDict(metaObj, bRefreshBuff=False):
    ...
    #            print 'EXCEPTION item_cls.value.field', item_cls.name
        ...
        log.fatal(u'Ошибка в getSpravBuffDict')
...
def getTypFldDict(metaObj):
    ...
    for i, item_cls in enumerate(lst):
        ...
        try:
            ...
            log.fatal(u'Ошибка <%s>' % item_cls.name)
...
#    progress.icOpenProgressBar("Выборка",0,100)
    ...
    sql = '''
SELECT 
    substr(codt, 1,3) as codt, 
    reg, 
    substr(mens, 2, 4) as mens, 
    sum(summa) as summa, 
    sum(kolf) as kolf
FROM (
        select *
...
def getChildTypesByStructPlan(defPlanLst, typ):
    ...
    try:
        ...
        log.fatal(u'Не корректный тип узла плана <%s>: %s' % (typ, defPlanLst))
...
def MetaPlanZeroSum(metaTree, Year_, Month_, aggregate_ctrl=None):
    ...
    for source_tab_name, source_tab in source_tabs.items():
        ...
        src_query = '''
SELECT 
    substr(dtoper, 1,4) as year,
    substr(dtoper, 6,2) as month,
    dtoper,
    substr(codt, 1,3) as vidprod, 
    reg, 
    substr(mens, 2, 4) as mens,
    sum(kolf) as kolf, 
    sum(summa) as summa
FROM %s 
WHERE 
    substr(dtoper, 1, 4)='%s' AND substr(dtoper, 6, 2)='%s'
group by year,month,dtoper,vidprod,reg,mens
ORDER BY year,month,dtoper
        ''' % (source_tab_name, Year_, Month_)
        log.debug(u'SQL <%s>' % src_query)
        ...
        for rec in recs:
            ...
            if not (None in value_path):
                ...
                log.warning(u'<%s : %s>' % (summa_path, rec))
        ...
        if value_path[0] in parentMetaObj._children.keys():
            ...
            if container_metaitems:
                ...
                try:
                    ...
                    log.fatal(u'Ошибка <%S>' % container_metaitems[0].name)
                    # pass
...
SELECT 
    substr(dtoper, 1, 4) as year,
    substr(dtoper, 6, 2) as month,
    dtoper,substr(codt, 1, 3) as vidprod, 
    reg, 
    substr(mens, 2, 4) as mens,
    sum(kolf) as kolf, 
    sum(summa) as summa
...
def refreshSumm(metaTree=None, year=None, month=None):
    ...
    log.info(u'Разнос сумм за <%s> месяц <%s> года.' % (month, year))
    if metaTree is None:
    ...
    # metaTree.SaveAllChildren()
    log.info(u'Окончание разноса сумм за <%s> месяц <%s> года.' % (month, year))
    return year, month
...
#                    (str(month), year), None, 'Получаем дерево базового плана', 10)
    ...
    if not t:
        ...
        try:
            ...
            msg = u'Элемент базового плана %s->%s не найден' % (mcod, ycod)
            log.fatal(msg)
            msgbox.MsgBox(parent, msg)
            return None
    ...
    for indx, r in enumerate(t):
        ...
        par = filter(lambda x: x is not None, par)
        mt[indx] = (str(par), val)
...
def set_value(metaObj, value):
    ...
    for key, val in value['analitic'].items():
        if key in metaObj.value.analitic:
    ...
    for key, val in value['zayavki'].items():
        if key in metaObj.value.zayavki:
...
def recount_fact(metaObj, bIndicator=True, bSave=True):
    ...
    if not metaObj.isRoot():
        ...
        # print('recount_fact::::',metaObj.name, metaObj.value.analitic['summa'], metaObj.value.zayavki['summa'], metaObj.value.pay['summa'])
